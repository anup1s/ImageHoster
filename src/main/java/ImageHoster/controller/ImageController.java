...
public class ImageController {
	...
	public String showImage(@PathVariable("imageId") Integer imageId, @PathVariable("title") String title,
		...
		model.addAttribute("comments", findAllComments(image));
		return "images/image";
	}
	...
	private List<Comment> findAllComments(Image image) {

		List<Comment> comments = new ArrayList<Comment>();
		try {
			comments = commentService.fetchAllCommentsForImage(image);
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("No comments found for image id " + image);
		}
