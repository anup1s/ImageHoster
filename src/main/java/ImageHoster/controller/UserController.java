...
public class UserController {
	...
	private static final String PASSWORD_ERR_MSG = "Password must contain atleast 1 alphabet, 1 number & 1 special character";
	...
	public String registerUser(User user, Model model) {
		...
		if (isValidPasswordUpgrad(user.getPassword())) {
			userService.registerUser(user);
			return "users/login";
		}

		model.addAttribute("User", user);
		model.addAttribute("passwordTypeError", PASSWORD_ERR_MSG);
		return "users/registration";
	}
	...
	public static boolean isValidPasswordUpgrad(String password) {

		// Regex to check valid password.
		String regex = "^(?=.*[0-9])" + "(?=.*[A-Za-z])" + "(?=.*[@#$%^&+=])" + "(?=\\S+$).{3,3000}$";
